...
这份文档用于沉淀 Agent 调优过程，重点记录 bad case、问题定位、修改策略和复测结果。它可以作为面试时说明“我怎么调 Agent”的证据材料。
...
- 状态：已验证
- 场景：AIOps 分析结束时，最终输出需要直接给用户阅读，而不是暴露中间 JSON。
...
- 状态：已验证
- 场景：同类告警再次出现时，Agent 应先读取历史经验。
- 原问题：原链路只看当前告警、知识库和日志，不能复用历史处置结果。
...
下面这些样例用于后续继续扩展调优证据链。每条完成后都应补充 before / after 和复测结果。
...
- 可能原因：缺少“证据不足”输出约束。
- 建议修改：工具连续失败或空结果时，必须输出“不足以确认根因”，并给出下一步取证建议。
...
- 典型现象：报告有日志证据和根因，但缺少短期止血和长期修复建议。
- 可能原因：报告模板缺少动作字段。
...
有 before / after 的记录可以作为调优证据；只有想法的内容保留在样例池中，作为后续优化计划。
